...
**Answer:** 
- **Synchronous methods** block the event loop until completion, preventing other code from executing.
- **Asynchronous methods** use callbacks, promises, or `async/await` to allow other operations to proceed while waiting for a task to finish.
...
**Answer:** Both are used to create child processes.
- **`spawn()`** launches a new process with a given command.
- **`fork()`** is a special case of `spawn()` that creates a new Node.js process and enables communication between parent and child via IPC (Inter-Process Communication) channels.
...
**Answer:** 
- **Callbacks** are functions passed as arguments to handle asynchronous results, but can lead to "callback hell" if nested deeply.
- **Promises** provide a cleaner, more manageable way to handle async operations, supporting chaining and error handling via `.then()` and `.catch()`.

### 10. How do you ensure scalability in a Node.js application?
**Answer:**
- Use clustering to take advantage of multi-core systems.
- Implement load balancing (e.g., with Nginx).
...
**Answer:**
- Regularly audit dependencies using tools like `npm audit` or `snyk`.
- Specify precise version ranges in `package.json`.
- Use `package-lock.json` to lock down versions.
